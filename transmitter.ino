/*
    Copyright (C) 2025  Dan Joel Sahaya
...
/*
  Uses code from the following example:
...
void loop() {
  if (nunchuk_read()) { 
    angle=nunchuk_joystick_angle();
    if (angle<0.0){
      angle+=M_PI*2.0;
    }
    angle=angle*100.0;
    if (angle==0 && nunchuk_joystickX()!=0){
        angle=1;
    }
  } 
  if (nunchuk_buttonZ()==1){//if button pressed, reset position
    angle=1000;
    ang=angle;
  }
  ang = angle; // Read Joysticks X-axis
  if (nunchuk_buttonC()==1){//if button pressed, stop signal
    if (isOn){
        isOn=false;
        delay(500);
    }
  ...
  ang.toCharArray(xyData, 5); // Put the String (angle) into a character array
  radio.write(&xyData, sizeof(xyData)); // Send the array data (angle) to the other NRF24L01 modile
  Serial.println(ang);
  delay(10);
}
